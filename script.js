...
var curSong = document.getElementById("mySong");
var curSongLi;
var preTrackNo = -1;
var prePlaylist;
var trackNo, songUrl, imgSrc, articts, songName;
var isReplay = false,
    isCycle = false,
    isPlayList = false;
...
function profile(trackNo, songUrl, imgSrc, articts, songName, isPlayList, listNo) {
    ...
    document.getElementById('singerNameScr').innerText = articts;
    ...
    if (isPlayList) {
        ...
        preTrackNo = Number(trackNo);
        prePlaylist = songList;
    } else {
        if (preTrackNo != -1) {
            prePlaylist[preTrackNo].style.border = "2px solid rgb(196, 195, 195)";
        }
...
curSong.addEventListener("ended", function() {
    if (isPlayList) {
        if (isReplay) {
            songList[trackNo].click();
        } else if (isCycle) {
            if ((songList.length - 1) != Number(trackNo)) {
                songList[++trackNo].click();
            } else if ((songList.length - 1) == Number(trackNo)) {
                songList[0].click();
            }
        }
    } else { curSong.play(); }
...
function btnleft() {
    if (isPlayList) {
        if (isReplay) { songList[trackNo].click(); } else if (isCycle) {
...
function btnright() {
    if (isPlayList) {
        if (isReplay) { songList[trackNo].click(); } else if (isCycle) {
            if ((songList.length - 1) != Number(trackNo)) { songList[++trackNo].click(); } else if ((songList.length - 1) == Number(trackNo)) { songList[0].click(); }
        }
    } else { curSong.play(); }
}
/* Replay button*/
...
function pausebyclient() {
    curSong.pause();
    icon.style.backgroundImage = "url('img/play.png')";
}

function playbyclient() {
    curSong.play();
    icon.style.backgroundImage = "url('img/pause.png')";
}
/*-----------------profiles end------------------*/
...
var audioTrack = document.getElementById("audioTrack"); //musci slider 
...
function changeVolume() { //audio functionality successfully
    if (!isMuted) {
        ...
        console.log(volumeValue / 10);
        audio.volume = ((volumeValue) / 10);
        if (volumeValue == 0) {
            document.getElementById("speaker").src = "img/mute.png";
        } else if (volumeValue > 7) {
            document.getElementById("speaker").src = "img/speaker2.png";
        } else if (volumeValue > 4) {
            document.getElementById("speaker").src = "img/speaker1.png";
...
function setFirst_End_Max_time() {
    ...
    if (flag) {
        startofSong.innerHTML = "0 : 00";
        flag = false;
    }
} //set end time when user play stating first time

function timeUpdate() {
    audioTrack.value = Number(audio.currentTime);
    startofSong.innerHTML = time_convert(Math.floor(audio.currentTime));
} //set current time to track 
...
} /* set current seeking by user  to music track  */